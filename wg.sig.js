...
/*	    if(bindings[elem.guid] && bindings[elem.guid].focus){
	...
	if(chld === undefined) sChildren = sig.constant([]);
	    ...
	    console.log("COMBINE", chld);
	    sChildren = sig.combine(chld, true);
	}
	else { console.log("SIG", chld); sChildren = chld; }
	return sChildren;
    }
	...
	function rebind(elem){
	    elem.bind[name] = function(__){ console.log("TOGGLE REBIND"); return s }; 
	    return elem; 
	}
...
/*    function spec(){
	...
	chosenDisplay.clicked().lift(function(x){ console.log("CHOSENDISPLAY", x); });
